...
const Todos: NextPage = (): JSX.Element => {
  ...
  return (
    ...
    <div   >
      ...
      <div><AppNavBar createNewTodo setCreateNewTodo={setCreateNewTodo} /></div>

      {createNewTodo && <CreateTodoForm createNewTodo={createNewTodo} setCreateNewTodo={setCreateNewTodo} />}
      <div className={styles.grid}>

        {!loading ? (todos.length > 0 ?
          todos.map((todo: TodoItem, index) => <TodoCard key={index} index={index} todo={todo} />) :
          <div><h1>No Todos...</h1></div>) :
          <Loading type="points"><Text color='white' >Fetching List</Text></Loading>}
